...
// each pizeo is stateful, they need to remember if they are sampling inside the
// debounce time, what the peak measurement of any particular strike was, etc.
struct PiezoState {
...
private:
  ...
  // helper: selects a given channel on the multiplexer (using the select
  // outputs)
  void selectMuxChannel(uint8_t channel);

  // process each piezo reading and update it's state
  void processPiezoReading(uint8_t piezo_index, uint16_t reading,
